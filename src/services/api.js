...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || 'https://vaagaimcqbk.vinothvk.in';
...
export const authApi = {
  loginWithGoogle: async (token, mockUser = null) => {
    ...
    const response = await apiInstance.post('/api/auth/google', { token, mockUser });
  ...
  loginWithRegNo: async (regNo, extraDetails = {}) => {
    ...
    const response = await apiInstance.post('/api/auth/examiner-login', { regNo, ...extraDetails });
    return response.data;
  },
  getProfile: async () => {
    if (USE_MOCK_API) {
...
export const adminApi = {
  ...
  uploadQuestions: async (testId, file, imageFiles = []) => {
          ...
          try {
            ...
            const questions = rows.map((row) => {
              ...
              Object.keys(row).forEach(key => {
                ...
                else if (cleanKey === 'option a' || cleanKey === 'optiona' || cleanKey === 'a') optionA = val;
                else if (cleanKey === 'option b' || cleanKey === 'optionb' || cleanKey === 'b') optionB = val;
                else if (cleanKey === 'option c' || cleanKey === 'optionc' || cleanKey === 'c') optionC = val;
                else if (cleanKey === 'option d' || cleanKey === 'optiond' || cleanKey === 'd') optionD = val;
                else if (cleanKey === 'correct answer' || cleanKey === 'correctanswer' || cleanKey === 'correct') correctAnswer = val.toUpperCase();
                else if (['image', 'image url', 'imageurl', 'image_url', 'image file'].includes(cleanKey)) imageUrl = val;
              });

              return { question, optionA, optionB, optionC, optionD, correctAnswer, imageUrl };
            });

            const result = await mockAdminApi.uploadQuestions(testId, questions);
            resolve(result);
          } catch (err) {
            reject(new Error('Failed to parse Excel file on client: ' + err.message));
          }
        };
    ...
    const response = await apiInstance.post('/api/admin/upload-questions', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    return response.data;
  },
  getResults: async (search = '', testId = '', page = 1, limit = 10) => {
    if (USE_MOCK_API) return mockAdminApi.getResults(search, testId, page, limit);
  ...
  updateQuestion: async (id, fields, imageFile = null) => {
    ...
    if (imageFile) formData.append('image', imageFile);
    const response = await apiInstance.put(`/api/admin/question/${id}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    return response.data;
  },
  ...
  deleteDuplicateQuestions: async () => {
    const response = await apiInstance.delete('/api/admin/questions/duplicates');
    return response.data;
  },
  // ── Examiner Registrations (CRUD & Import) ──────────────────────────────
  getRegistrations: async (search = '', className = '', page = 1, limit = 10) => {
  ...
  updateRegistration: async (refNo, data) => {
    const response = await apiInstance.put(`/api/admin/registrations/${refNo}`, data);
    return response.data;
  },
  deleteRegistration: async (refNo) => {
    const response = await apiInstance.delete(`/api/admin/registrations/${refNo}`);
    return response.data;
  },
  importRegistrations: async (file) => {
    const formData = new FormData();
    formData.append('file', file);
    const response = await apiInstance.post('/api/admin/registrations/import', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    });
    return response.data;
  }
...
export const examinerApi = {
  ...
  submitTest: async (testId, answers, timeTaken, email, forceZeroScore = false) => {
    if (USE_MOCK_API) return mockExaminerApi.submitTest(testId, answers, timeTaken, email, forceZeroScore);
    const response = await apiInstance.post('/api/examiner/submit', { testId, answers, timeTaken, forceZeroScore });
    return response.data;
  },
