...
import { Award, CheckCircle, XCircle, ArrowLeft, RefreshCw, AlertCircle } from 'lucide-react';
...
export const Result = () => {
  ...
  useEffect(() => {
    const fetchResult = async () => {
      try {
        ...
        const msg = err.response?.data?.error || err.message || 'Failed to load graded scorecard.';
        setErrorMsg(msg);
        toast.error(msg);
      } finally {
        setLoading(false);
  ...
  if (!result) {
    ...
    return (
      <div className="py-16 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md mx-auto px-6">
        ...
        <Link to="/examiner/tests" className="px-4 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-xs transition-colors">
  ...
  let feedbackDesc = 'Review the incorrect questions below to improve your understanding of the concepts.';
  
  if (percent >= 90) {
    feedbackTitle = 'Outstanding Performance!';
    feedbackDesc = 'You have demonstrated absolute mastery over these assessment subjects. Excellent job!';
  } else if (percent >= 70) {
    feedbackTitle = 'Great Effort!';
    feedbackDesc = 'You scored well on this assessment. Minor review of concepts will get you to perfection.';
  }
    ...
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-md text-center space-y-6">
        ...
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white tracking-tight">{result.testName}</h1>
          <p className="text-slate-450 dark:text-slate-500 text-xs">Evaluated on {new Date(result.submittedAt).toLocaleString()}</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center gap-6 py-4">
          <div className="flex items-baseline gap-1 bg-slate-50 dark:bg-slate-850 border border-slate-100 dark:border-slate-800 px-6 py-4 rounded-3xl shadow-inner">
            <span className="text-4xl font-extrabold text-slate-800 dark:text-white">{result.score}</span>
            <span className="text-slate-400 text-sm">/ {result.total}</span>
            <span className="text-xs text-slate-450 font-bold ml-2">Questions</span>
          </div>

          <div className={`px-6 py-4 rounded-3xl border font-bold text-lg shadow-sm ${passed ? 'bg-emerald-50 dark:bg-emerald-950/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/30' : 'bg-rose-50 dark:bg-rose-950/10 text-rose-600 dark:text-rose-450 border-rose-100 dark:border-rose-900/30'}`}>
            {percent.toFixed(0)}% Score
          </div>
        </div>

        <div className="max-w-md mx-auto pt-2">
          <h3 className="font-bold text-slate-800 dark:text-slate-200">{feedbackTitle}</h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">{feedbackDesc}</p>
        </div>
      </div>
      ...
      {result.questions && result.questions.length > 0 && (
        <div className="space-y-6">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white font-sans tracking-tight">Grader Response Analysis</h3>

          <div className="space-y-6">
            {result.questions.map((q, idx) => {
              // Find matching selected answer
              const answerObj = result.answers?.find(a => a.questionId === q.id);
              const selectedOption = answerObj?.selectedOption || '';
              const isCorrect = selectedOption.toUpperCase() === q.correctAnswer.toUpperCase();
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold flex items-center gap-1 shrink-0 ${isCorrect ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/20' : 'bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 border border-rose-100 dark:border-rose-900/20'}`}>
                      {isCorrect ? <CheckCircle size={12} /> : <XCircle size={12} />}
                      {isCorrect ? 'Correct' : selectedOption === '' ? 'Unanswered' : 'Incorrect'}
                    </span>
                  </div>
                  ...
                  {q.imageUrl && (
                    <div className="overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 max-h-[500px] bg-slate-50/50 flex justify-start">
                      <img 
                        src={q.imageUrl.startsWith('http') ? q.imageUrl : `${import.meta.env.VITE_API_BASE_URL || 'https://vaagaimcqbk.vinothvk.in'}${q.imageUrl}`} 
                        alt="Diagram" 
                  ...
                  <div className="grid grid-cols-1 gap-2 pt-2 text-sm">
                    {/* User Selection */}
                    <div className={`p-3 rounded-xl border ${isCorrect ? 'bg-emerald-50/40 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-900/20 text-emerald-800 dark:text-emerald-350' : 'bg-rose-50/40 dark:bg-rose-950/10 border-rose-100 dark:border-rose-900/20 text-rose-800 dark:text-rose-350'}`}>
                      <span className="font-bold">Your Response: </span>
                      {selectedOption ? (
                        <span><strong>[{selectedOption}]</strong> {optionTexts[selectedOption]}</span>
                      ) : (
                        <span className="italic font-normal">No option selected</span>
                      )}
                    </div>
                    ...
                    {!isCorrect && (
                      <div className="p-3 rounded-xl border bg-emerald-50/40 dark:bg-emerald-950/10 border-emerald-100 dark:border-emerald-900/20 text-emerald-800 dark:text-emerald-350">
                        ...
                        <span><strong>[{q.correctAnswer}]</strong> {optionTexts[q.correctAnswer]}</span>
                      </div>
                    )}
                    ...
                    {q.explanation && (
                      <div className="p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-850/60 text-slate-650 dark:text-slate-350">
                        <span className="font-bold block text-slate-850 dark:text-slate-250 mb-1">Explanation:</span>
