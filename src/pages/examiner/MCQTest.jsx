...
export const MCQTest = () => {
    ...
    const handleBlur = () => {
      if (document.fullscreenElement) {
        document
          .exitFullscreen()
          .catch((err) =>
            console.error("Failed to exit fullscreen on blur:", err),
          );
      }
    };
      ...
      intervalId = setInterval(() => {
        setFullscreenCountdown((prev) => {
          if (prev <= 1) {
            ...
            toast.error(
              "Security violation: Failed to return to fullscreen. Test submitted with 0 marks.",
              {
                duration: 10000,
                icon: "🚨",
              },
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      ...
      if (
        ...
        e.stopPropagation();
        toast.error(
          "Shortcut keys (Alt, Ctrl, Shift, Windows, Print Screen) are disabled during this assessment!",
          {
    ...
    const handleKeyUp = (e) => {
      if (e.key === "PrintScreen" || e.key === "Snapshot") {
        ...
        toast.error(
          "Screenshots are strictly prohibited during the assessment!",
          {
  ...
  const submitExamAnswers = async (forceZeroScore = false) => {
    ...
    try {
      ...
      const result = await examinerApi.submitTest(
        id,
        formattedAnswers,
        timeSpent,
        user.email,
      ...
      if (forceZeroScore) {
        toast.error(
          "Assessment terminated due to security violation. Graded score: 0",
        );
        navigate("/examiner/tests");
      } else {
        if (result.resultsPublished) {
          ...
          toast.success(
            "Assessment submitted successfully! Results are pending publication by the administrator.",
          );
          navigate("/examiner/tests");
        }
      }
  ...
  return (
    <div
      className={`mx-auto space-y-6 transition-all duration-350 ${started ? "max-w-6xl" : "max-w-3xl"}`}
    >
      {/* 1. START LOBBY BOARD */}
      {!started ? (
          ...
          {questions.length === 0 ? (
            <>
              ...
              <p className="text-rose-500 dark:text-rose-400 text-sm mt-3 font-semibold">
                No questions available matching your class level (Class{" "}
                {user?.class || "Universal"}).
              </p>
              <p className="text-slate-500 dark:text-slate-400 text-xs mt-2 leading-relaxed">
                Please contact your test administrator to configure your
                assigned class standard or questions.
              </p>
              <button
            ...
            <>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Ready for evaluation? Please confirm the conditions below.
              </p>
              ...
              <div className="mt-8 border border-slate-100 dark:border-slate-800/40 rounded-2xl p-5 bg-slate-50/50 dark:bg-slate-900/50 text-left text-xs text-slate-600 space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <span>
                    <strong>Duration:</strong> {test?.duration} minutes absolute
                    limit.
                  </span>
                </div>
                <div className="flex items-start gap-2.5">
                  <span className="w-1.5 h-1.5 bg-primary-500 rounded-full mt-1.5 shrink-0" />
                  <span>
                    <strong>Security:</strong> Fullscreen mode is enforced. Do
                    not leave the active window.
                  </span>
                </div>
                <div className="flex items-start gap-2.5">
                  <span className="w-1.5 h-1.5 bg-primary-500 rounded-full mt-1.5 shrink-0" />
                  <span>
                    <strong>Single Take:</strong> Re-takes are locked once
                    initialized.
                  </span>
                </div>
              </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            {!isFullscreen && (
              <div className="flex items-center justify-between p-4 bg-rose-50 dark:bg-rose-950/20 text-rose-700 dark:text-rose-405 border-2 border-rose-500 dark:border-rose-900/50 rounded-2xl text-xs shadow-md animate-pulse">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <div className="font-bold text-sm text-rose-800 dark:text-rose-400">
                      Security Alert: Fullscreen mode deactivated!
                    </div>
                    <div className="mt-1 text-slate-650 dark:text-slate-400">
                      ...
                      remaining, or your test will be terminated and submitted
                      with{" "}
                      <strong className="text-rose-650 dark:text-rose-400">
                        0 marks
                      </strong>
                      .
                    </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm flex items-center justify-between gap-4">
              <div className="flex-1 col-span-1">
                <h3 className="text-md font-bold text-slate-900 dark:text-white truncate max-w-sm sm:max-w-md">
                  {test?.name}
                </h3>
                {/* Progress bar */}
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-md space-y-6">
              ...
              <h2
                className={`${questionSizeClass} font-bold text-slate-900 dark:text-white leading-relaxed font-sans`}
              ...
              {activeQuestion.imageUrl && (
                <div className="relative mt-4 overflow-hidden rounded-2xl border border-slate-100 dark:border-slate-800 max-h-[600px] bg-slate-50/50 flex justify-center group">
                  <img
                    src={
                      activeQuestion.imageUrl.startsWith("http")
                        ? activeQuestion.imageUrl
                        : `${import.meta.env.VITE_API_BASE_URL || "https://vaagaimcqbk.vinothvk.in"}${activeQuestion.imageUrl}`
                    }
              ...
              <div className="grid grid-cols-1 gap-4 pt-4">
                {activeOptions.map((opt) => {
                  const isSelected =
                    selectedAnswers[activeQuestion.id] === opt.label;
                  return (
                    <button
                      key={opt.label}
                    ...
                    >
                      ...
                      <div
                        className={`${choiceBadgeClass} rounded-full flex items-center justify-center font-bold shrink-0 border mt-0.5 ${isSelected ? "bg-primary-600 border-primary-600 text-white shadow-sm" : "border-slate-350 dark:border-slate-700 text-slate-500 bg-white dark:bg-slate-900"}`}
                      >
                        {opt.label}
                      </div>
                      <span
                        className={`${choiceTextClass} font-semibold leading-relaxed`}
                      >
                        {opt.text}
                      </span>
                    </button>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-md space-y-4">
              <div className="border-b border-slate-100 dark:border-slate-800/60 pb-3">
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">
                  Quickly select or navigate questions
                </p>
              </div>
              ...
              <div className="grid grid-cols-5 gap-2">
                {questions.map((q, idx) => {
                  ...
                  let btnClass =
                    "w-full aspect-square flex items-center justify-center font-mono font-bold text-xs rounded-xl transition-all ";
                  if (isCurrent) {
                    btnClass +=
                      "bg-primary-600 text-white shadow-md ring-2 ring-primary-500/20";
                  } else if (isAnswered) {
                    btnClass +=
                      "bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 text-emerald-600 dark:text-emerald-450";
                  } else {
                    btnClass +=
      ...
      <ConfirmModal
        ...
        title={
          unansweredCount > 0 ? "Submit Unfinished Exam?" : "Submit Assessment?"
        }
        message={
          unansweredCount > 0
